import hashlib, json, inspect, locale, operator, os, pydoc, re
...
def getdoc(obj):
    ...
    pkgname = (modname_bits and modname_bits[0] or '')
    if objname is None:
        objname = '.'.join(modname_bits[1:] + [obj.__name__])
    language = 'ja_JP' # locale.getdefaultlocale()[0]
    catalog = None
    if pkgname not in PY_IHELP_CACHE:
        catalog_fn = os.path.join(
            PY_IHELP_ROOT, '%s.%s.ihelp' %(pkgname, language))
        if os.path.exists(catalog_fn):
            ...
            catalog = {}
        # PY_IHELP_CACHE[pkgname] = catalog
    #catalog = PY_IHELP_CACHE[pkgname]
    def docstrip(d):
        return d and re.sub('^ *\n', '', d.rstrip()) or ''
    if catalog:
        if objname in catalog:
            last_valid = None
            for signature, valid, translated in catalog[objname]:
                if valid:
                    last_valid = translated
                    if signature==hashlib.md5(doc).hexdigest():
                        return docstrip(translated)
            if last_valid:
                return docstrip(last_valid)
    return docstrip(doc)
...
class GetDoc(object):
    ...
    def load_catalog(self, pkgname):
        warnings.warn('Loading %s\n' %pkgname)
        catalog_fn = os.path.join(
            PY_IHELP_ROOT, '%s.%s.ihelp' %(pkgname, self.language))
        if os.path.exists(catalog_fn):
            try:
                catalog = eval(
                    unicode(open(catalog_fn, 'rb').read(), 'utf-8', 'ignore'),
                ...
                warnings.warn('Warning: error while reading catalog.\n'+'reason= '+str(e))
                catalog = {}
        else:
            catalog = {}
        return catalog

    def get_catalog(self, pkgname):
        if pkgname not in self.cache:
            self.cache[pkgname] = self.load_catalog(pkgname)
        return self.cache[pkgname]

    def getdoc(self, object):
        """Get the doc string or comments for an object."""
        modname_bits = str(inspect.getmodule(object).__name__).split('.')
        pkgname = modname_bits and modname_bits[0] or ''
        # doc = inspect.getdoc(object) or inspect.getcomments(object)
        # doc = doc and re.sub('^ *\n', '', doc.rstrip()) or ''
        doc = pydoc._getdoc(object)
        catalog = self.get_catalog(pkgname)
        if catalog:
            objname = '.'.join(modname_bits[1:] + [object.__name__])
            warnings.warn('Finding %s\n' %objname)
            if objname in catalog:
                warnings.warn('Found %s\n' %objname)
                last_valid = None
                for signature, valid, translated in catalog[objname]:
                    warnings.warn('Trying %s\n' %([signature, valid]))
                    if valid:
                        warnings.warn('Valid %s\n' %signature)
                        last_valid = translated
                        if signature==hashlib.md5(doc).hexdigest():
                            warnings.warn('Found valid %s\n' %signature)
                            return translated
                if last_valid:
                    warnings.warn('Warning: this translation may be obsolete')
                    return last_valid
        return doc
...
def install(language=locale.getdefaultlocale()[0] or 'en'):
    #getdoc = GetDoc(language).getdoc
    # snatch
    #pydoc.getdoc = getdoc
    # pydoc.pipepager = pipepager
    #from site import _Helper
    import __builtin__
    __builtin__.ihelp = Helper(sys.stdin, sys.stdout)
