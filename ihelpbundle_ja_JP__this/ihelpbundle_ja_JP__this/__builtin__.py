...
注意: None は `nil\' オブジェクトを表現します。また、 Ellipsis はスラ
イスの `...\' を表します。"""),
...
モジュールを import します。引数 globals は import 時のコンテキストを
決定するためだけに使われ、変更されることはありません。locals は今のと
ころ使われていません。 fromlist は ``from name import ...\'\' のエミュ
レーションをするときの import する名前のリストです。 ``import name\'\'
をエミュレーションするときには、空のリストにします。モジュールをパッケー
ジから import する場合、 fromlist を空にしておくと、
__import__(\'A.B\', ...) はパッケージ A を返します。 fromlist が空でな
ければ、サブモジュール B を返します。 level は import の形式が絶対
import か、相対 import かを決めます。 -1 にすると、元々の import 戦略、
すなわち絶対と相対の両方の import を試みます。 0 にすると絶対 import
のみ、正の値にすると、現在のモジュールから指定した数だけ上のディレクト
リからの相対でモジュールを探します。"""),
...
iterable のすべての要素 x について、 bool(x) が True である場合に True
を返します。"""),
...
iterable のいずれかの要素 x について bool(x) が True のとき True を返
します。"""),
...
args にタプルを指定すると固定引数として、 kwargs に辞書を指定するとオ
プションのキーワード引数として使います。クラスや、 __call__() メソッド
を備えたインスタンスも callable です。

リリース 2.3 で撤廃されました。 apply の代わりに、
function(*args, **keywords) を使ってください。"""),
...
"""basestring は直接インスタンス化できません。このクラスは str や
unicode のベースクラスです。"""),
...
'basestring.__eq__': [
('29276e56c1f9b89c0e4975783c563226', True, 
"""x.__eq__(y) <==> x==y"""),
...
'basestring.__ge__': [
('a40b7fe72a380879d7602be9b639db1b', True, 
"""x.__ge__(y) <==> x>=y"""),
...
('efa80438e88a77b4eba07ebe2d9ceb85', True, 
...
'basestring.__gt__': [
('43f3c9de3ba1d0a78dea8d37b57bff37', True, 
"""x.__gt__(y) <==> x>y"""),
...
'basestring.__hash__': [
('63a96d56ab20e8afcaf282823c66d409', True, 
"""x.__hash__() <==> hash(x)"""),
...
'basestring.__le__': [
('486eb7462a194cfb7ba27a685d3c6ec3', True, 
"""x.__le__(y) <==> x<=y"""),
...
'basestring.__lt__': [
('803809016499ca6afb70031b8fc55efe', True, 
"""x.__lt__(y) <==> x<y"""),
...
'basestring.__ne__': [
('1ffca5c8e0f2a871a1986db525d30469', True, 
"""x.__ne__(y) <==> x!=y"""),
...
'basestring.__repr__': [
('d7ecf995070b7d400aad745f917dd8f8', True, 
"""x.__repr__() <==> repr(x)"""),
...
'basestring.__str__': [
('5f53e31d167624726f84cafe0dfc5af2', True, 
"""x.__str__() <==> str(x)"""),
...
引数に指定したオブジェクトを参照する、新たなバッファ (buffer) オブジェ
クトを生成します。生成されたバッファは、対象オブジェクトの先頭から(ま
たは offset から) のスライスを参照します。このスライスの範囲は、対象オ
ブジェクトの末尾まで (または size に指定した長さまで) です。"""),
...
'buffer.__delitem__': [
('367ba526f8846abe6af406894f245e61', True, 
...
'buffer.__getitem__': [
('5b1a2fedeaae4789f80b680bc5ab6529', True, 
"""x.__getitem__(y) <==> x[y]"""),
...
'buffer.__len__': [
('3d6e075527d9b8e03b83a311002c24c1', True, 
"""x.__len__() <==> len(x)"""),
...
'buffer.__mul__': [
('81afa97dd875518810269060ffb7d938', True, 
"""x.__mul__(n) <==> x*n"""),
...
'buffer.__rmul__': [
('3705f779f36813dd93b82ce068898521', True, 
"""x.__rmul__(n) <==> n*x"""),
...
'buffer.__setitem__': [
('8c8d81a007fa9ee02ec1da163dab9ff7', True, 
...
"""T.__new__(S, ...) は、 T のサブタイプ S の新たなオブジェクトを生成
して返します。"""),
...
それぞれ、以下の形式のデータから、変更可能なバイト列 (bytearray) を生
成します。:
...
('3e4d608788a476ebb6efff2e2c3258bf', True, 
...
'bytearray.__iadd__': [
('c6bf05c3c7c62466ba0e5fe88092ca3c', True, 
"""x.__iadd__(y) <==> x+=y"""),
...
'bytearray.__imul__': [
('716204859290aad65b3c9d6cf8b957f8', True, 
"""x.__imul__(y) <==> x*=y"""),
...
位置するような文字列を生成して返します。 fillchar を省略した場合、スペー
スを代わりに使います。"""),
...
start と end はスライス表記と同じ方法で指定でき、省略するとそれぞれ B
の先頭と末尾を表します。"""),
...
encoding の名前で登録されている codec を使って、B をデコードして返しま
す。 encoding を省略すると、デフォルトエンコーディングを使います。エラー
を指定すると、デコード中に発生したエラーの処理スキームを変更できます。
デフォルトの設定は \'strict\' で、すべてのエンコーディングエラーに対し
て UnicodeDecodeError を送出します。他には \'ignore\' と \'replace\'
を指定できます。また、 UnicodeDecodeError を処理できるエラーハンドラを
定義して codecs.register_error である名前で登録しておけば、その名前も
指定できます。"""),
...
B が suffix で終わる場合には True を、それ以外の場合には False を返し
ます。start を指定すると検索の開始位置を、 end を指定すると検索の終了
位置を指定できます。 suffix を文字列のタプルにすると、複数の文字列を調
べられます。"""),
...
変換して返します。 tabsize を指定しない場合、タブの幅を 8 スペースとみ
なします。"""),
...
B の中の範囲 s[start, end] で部分列 sub を探し、最初に見つかった場所の
インデクスを返します。 start と end は省略可能で、スライス表記と同じ指
定方法を使えます。見つからなければ -1 を返します。"""),
...
16 表記で数を表現している文字列からbytearray オブジェクトを生成して返
します。数の間にスペースが入っていてもかまいません。


例: bytearray.fromhex(\'B9 01EF\') ->
...
B.find() と似ていますが、 sub が見つからないときに ValueError を送出し
ます。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が英数字である場合に
True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字がアルファベットである
場合に True を返します。それ以外の場合には False を返します。
...
B が少なくとも 1 文字からなり、そのすべての文字が数字である場合に True
を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が小文字である場合に
True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が空白文字である場合に
True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、タイトルケースである場合、すなわち先頭
の文字が大文字で、それ以降の文字が小文字である場合に True を返します。
...
B が少なくとも 1 文字からなり、そのすべての文字が大文字である場合に
True を返します。それ以外の場合には False を返します。"""),
...
iterable_of_bytes の任意個数の bytearray オブジェクトを B で結合したバ
イト列を生成して返します。"""),
...
B を幅 width の文字列中に左寄せに配置した文字列を生成して返します。文
字列は fillchar に指定した文字でパディングされます。 fillchar を省略す
ると、スペースを使います。"""),
...
B のコピーを生成し、先頭から、引数 bytes に指定した文字を除去して返し
ます。引数を省略すると、先頭の ASCII 空白文字を除去します。"""),
...
S の中でセパレータ sep を探し、見つかった場合には、セパレータ以前前の
文字列、セパレータ本体、セパレータ以降の文字列からなるタプルを返します。
セパレータが見つからなかった場合には、 S と二つの空の bytearray からな
るタプルを返します。"""),
...
B の index の位置にある要素を一つ除去し、除去した要素を返します。index
を省略すると、末尾の値を pop します。"""),
...
B のコピーを生成し、 old を new に置き換えて返します。オプションの引数
count を指定した場合、先頭から count 回までで置き換えを止めます。"""),
...
B の中で sub を探し、最後に見つかった位置のインデクスを返します。sub
の検索範囲は s[start,end] です。 start および end は省略可能で、スライ
ス表記と同じ方法で指定できます。
検索に失敗すると -1 を返します。"""),
...
B.rfind() に似ていますが、見つからなかった場合に ValueError を送出しま
す。"""),
...
B を幅 width の文字列中に右寄せに配置した文字列を生成して返します。文
字列は fillchar に指定した文字でパディングされます。 fillchar を省略す
ると、スペースを使います。"""),
...
以前前の文字列、セパレータ本体、セパレータ以降の文字列からなるタプルを
返します。セパレータが見つからなかった場合には、 S と二つの空の
bytearray からなるタプルを返します。"""),
...
引数 sep に含まれる文字を区切り文字として、 B を分割したリストを生成し
て返します。分割は B の末尾から先頭に向けて実行されます。 sep を指定し
ない場合、 ASCII 空白文字 (スペース、タブ、復帰、改行、行送り、水平タ
ブ) を使って B を分割します。 maxsplit を指定すると、maxsplit 個まで分
割した時点で分割を止めます。"""),
...
B の末尾から、 bytes に指定した文字を除去して返します。bytes を省略す
ると、 ASCII 空白文字を除去します。"""),
...
"""B.split([sep[, maxsplit]]) -> bytearray の list

引数 sep に含まれる文字を区切り文字として、 B を 分割したリストを生成
して返します。 sep を指定しない場合、 ASCII 空白文字 (スペース、タブ、
復帰、改行、行送り、水平タブ) を使って B を分割します。maxsplit を指定
すると、 maxsplit 個まで分割した時点で分割を止めます。"""),
...
B を改行で区切ったリストを返します。 keepends に True を指定しない限り、
リストには改行文字を含めません。"""),
...
B の部分文字列が prefix に指定した文字列から開始する場合には True、そ
れ以外の場合には False を返します。引数 start と end には、検索の開始
位置と終了位置を指定します。省略すると、それぞれ B の先頭から末尾まで
を調べます。 prefix には、調べたい複数の文字列をタプルで指定できます。
"""),
...
文字列の先頭と末尾から、 bytes に含まれる文字を除去します。bytes 引数
を省略した場合、 ASCII の空白文字を除去します。"""),
...
内に含まれる文字をすべて除去し、残りの文字について、変換テーブル table
に従って文字を変換して返します。 table は長さ 256 のバイト列でなければ
なりません。"""),
...
B の左側にゼロを追加して、全体で幅 width にします。B の幅が width 以下
でも、 B を切り詰めません。"""),
...
呼出し可能オブジェクト (関数など) かどうかを判定して返します。クラスや、
__call__()  メソッドを持つインスタンスも呼出し可能です。"""),
...
序数が i の文字 1 つからなる文字列を返します。i の範囲は 0 <= i < 256
です。"""),
...
インスタンスメソッドの第一引数にインスタンスを渡すように、クラスメソッ
ドの第一引数には、クラスを渡すことになっています。クラスメソッドを宣言
するには、以下のようなイディオムを使いましょう:
...
(C().f() のような) インスタンスに対しても呼び出せます。後者の場合、ク
ラスメソッドにはインスタンスのクラス以外の情報は渡りません。クラスメソッ
ドをサブクラスに対して呼び出すと、第一引数にはサブクラスのクラスオブジェ
クトが渡されます。

クラスメソッドは C++ や Java の静的メソッドとは違います。静的メソッド
が必要なら、組込み関数 staticmethod を使ってください。"""),
...
'classmethod.__get__': [
('d26800418397c66c49e33dcfd65f7f03', True, 
...
"""compile(source, filename, mode[, flags[, dont_inherit]]) -> コードオブジェクト

ソースコード文字列 (Python モジュールや実行文、式) をコンパイルして、
exec 文や eval() で実行可能なコードオブジェクトに変換します。filename
引数は、実行時のエラーメッセージに使われます。 mode 引数は、モジュール
をコンパイルする場合は \'exec\' に、単一の (対話的に実行する) 式の場合
は \'single\' に、式の場合には \'eval\' にします。flags 引数を指定する
と、コードのコンパイルにどの future 文を影響させるかを制御できます。
dont_inherit 引数をゼロでない値にすると、 compile を呼び出す側のコード
で有効になっている future 文の効果をコンパイル処理に影響させません。
...
指定のアトリビュートをオブジェクトから除去します。 delattr(x, \'y\')
は ``del x.y\'\' と同じです。"""),
...
"""dict() は新しい空の辞書を返します。
dict(mapping) は新しい辞書を生成し、 mapping オブジェクトのキー/値ペア
を使って初期化します。
dict(seq) は新しい辞書を生成し、以下のような初期化を行います:
    d = {}
    for k, v in seq:
        d[k] = v
dict(**kwargs) は新しい辞書を生成し、キーワード引数リストの name=value
ペアで初期化します。例えば、 dict(one=1, two=2) のように使います。"""),
...
"""D.__contains__(k) は、 D にキー k があれば True を、そうでなければ
False を返します。"""),
...
"""D.__sizeof__() は D のメモリ上のサイズをバイト単位で返します"""),
...
"""D.clear() は D から全ての要素を除去し、None を返します。"""),
...
"""D.copy() は D の浅いコピーを返します。"""),

],


'dict.fromkeys': [
('c8aca1752b1e4999fea4cf14631926d5', True, 
"""dict.fromkeys(S[,v]) は 新たな辞書を返します。この辞書は S からキー
を引き継ぎます。値はすべて v に設定されます。 v のデフォルト値は None
です。"""),
...
'dict.get': [
('ad21d1fea6b5935438a580c62b681c93', True, 
"""D.get(k[,d]) は D 上にキー k があれば D[k], そうでなければ d を返し
ます。d のデフォルト値は None です。"""),

],


'dict.has_key': [
('2bd9db68d3dc63bd599247a9e5df09c1', True, 
"""辞書中にキー k があれば True, そうでなければ False を返します。"""),

],


'dict.items': [
('5dd74642fd0ff63f6f2ae972c3d9e3c5', True, 
"""辞書中の (key, value) ペアのタプルからなるリストを返します。"""),

],


'dict.iteritems': [
('3ff509be366199ffd446dd23a6161e08', True, 
"""辞書中の (key, value) ペアを列挙するイテレータを返します。"""),

],


'dict.iterkeys': [
('cbde69c2e295dc6b80abb5a9a4279335', True, 
"""辞書中のキーを列挙するイテレータを返します。"""),

],


'dict.itervalues': [
('33b6e1124fa89630ccfb758fa12a19eb', True, 
"""辞書中の値を列挙するイテレータを返します。"""),

],


'dict.keys': [
('36f2537aa81e7a32a3cb66082548de85', True, 
"""D のキーからなるリストを返します。"""),
...
'dict.pop': [
('e3bc5557355506f93dcd4bc6d19c70ec', True, 
"""D.pop(k[,d]) -> v

辞書からキー k を除去して、キーに対応する値を返します。キーが辞書中に
ない場合、 d を指定していれば返し、指定していなければ KeyError を送出
します。"""),
...
辞書から (key, value) のペアを一つ除去して、タプルで返します。 D が空
の場合には KeyError を送出します。"""),
...
"""D.setdefault(k[,d]) は D.get(k,d) と似ていますが、 k が D 上にない
場合は D[k]=d を実行します。"""),
...
"""D.update(E, **F) は D の要素を、辞書または iterable の E と F で更
新して、 None を返します。

E が .keys() メソッドを持つ場合は for k in E: D[k] = E[k], E が
.keys() メソッドを持たない場合は for (k, v) in E: D[k] = v です。どち
らの場合も、続いて for k in F: D[k] = F[k] を適用します。"""),
...
引数を指定した場合、引数に指定したオブジェクトの（一部の）アトリビュー
トと、オブジェクトを介してアクセスできるアトリビュートの名前からなるア
ルファベット順のリストを返します。
オブジェクトが __dir__ メソッドを備えている場合、名前の取得に __dir__
を使います。それ以外の場合には、以下のようなロジックを使います:
  モジュールオブジェクトの場合: モジュールのアトリビュートを返します。
  クラスオブジェクトの場合: クラスのアトリビュートの他に、ベースクラス
  のアトリビュートを再帰的に探索して返します。
  それ以外のオブジェクトの場合: オブジェクトのアトリビュート、オブジェ
  クトのクラスのアトリビュートの他に、ベースクラスのアトリビュートを再
  帰的に探索して返します。"""),
...
((x-x%y)/y, x%y) からなるタプルを返します。常に、 div*y + mod == x が
成り立ちます。"""),
...
"""enumerate(iterable) はイテレータを返します。このイテレータは、引数
iterable の各値について、値のインデクスと値自体からなるタプルを返しま
す。

enumerate オブジェクトを返します。 iterable はイテレーションをサポート
するオブジェクトでなければなりません。 enumerate オブジェクトは、
iterable から逐次値を取り出し、 (ゼロから始まる）インデクスと値からな
るタプルを生成して yield します。
(0, seq[0]), (1, seq[1]), (2, seq[2]), ... のようなインデクス付きのリ
ストを生成するときに便利です。"""),
...
"""x.next() は、次の値を返します。これ以上値がなければ StopIteration
を送出します。"""),
...
引数 source を、現在の名前空間 globals および locals の下で評価します。
source は Python の式を記述した文字列でも、compile() の返すコードオブ
ジェクトでもかまいません。
globals は辞書オブジェクトでなければなりません。 locals はマッピングオ
ブジェクトであればよく、指定しなければそれぞれ現在のグローバル名前空間
およびローカル名前空間を使います。
...
globals および locals は辞書で、デフォルト値はそれぞれ現在のグローバル
名前空間とローカル名前空間です。
globals だけを指定した場合、 locals には globals と同じ値を使います。
"""),
...
ファイルを開きます。 mode には \'r\', \'w\', \'a\' を指定でき、それぞ
れ読み出し(デフォルト値)、書き込みおよび追記を表します。書き込みまたは
追記モードでファイルを開く際、ファイルがまだ存在しなければ新たに作成さ
れます。書き込み用にファイルを開いた場合、既存のファイルは 0 バイトに
切り詰められます。ファイルをバイナリファイルとして開く場合は、モードに
\'b\' を付加してください。ファイルを読み書き同時で開くときには mode に
\'+\' を加えてください。
buffering 引数を指定した場合、 0 にするとバッファリングなし、 1 にする
と行単位のバッファリング、それ以上の値を指定した場合はバッファサイズを
表します。
ファイルを開くには、組み込みの open() 関数を使うよう勧めます。
ファイルや端末入力を開くときにユニバーサル改行サポートを有効にするには
\'U\' を追加してください。入力ファイル中の行末は Python 中では\'\\n\'
で表されます。また、このオプションを付けてファイルを開くと、オブジェク
トには \'newlines\' アトリビュートが付与されます。このアトリビュートの
値は、 None (まだ改行を読み込んでいない場合)、 \'\\r\', \'\\n\',
\'\\r\\n\', 読みだした改行文字の種類全てを含むタプルのいずれかをとりま
す。
...
"""close() はファイルを閉じ、 None または (場合によっては) 整数を返し
ます。


ファイルオブジェクトの .closed アトリビュートを True に設定します。ファ
イルを閉じると、それ以降の I/O 操作に使えなくなります。 close() は何度
でも呼び出してよく、重複して呼び出してもエラーを起こしません。ある種の
ファイルオブジェクト (例えば popen() で開いたもの) は、ファイルを閉じ
るときに終了ステータスを返すことがあります。"""),
...
"""isatty() は、ファイルが端末デバイスに接続されていれば True を返しま
す。"""),
...
'file.next': [
('288fd681cdf3b0a8149971d514e6e91e', True, 
"""x.next() はファイル中の次の文字を返します。ファイルの末尾なら
StopIteration を送出します。"""),
...
"""read([size]) は最大 size バイトまでをファイルから読み出し、文字列で
返します。

size 引数を負の値にするか省略すると、 EOF に到達するまでファイルを読み
ます。非ブロッキングモードでファイルを開いていると、size パラメタを指
定していても、求めるサイズより小さなデータを返すことがあるので注意して
ください。"""),
...
'file.readinto': [
('9cb9c29c58b05612f7cfac3381296711', True, 
"""readinto() はアンドキュメンテッド機能です。使わないでください。なく
なる予定です。"""),
...
'file.readline': [
('e5e0d7abdcef0951b0c6248b166e408e', True, 
"""readline([size]) はファイルから行読みだして文字列で返します。

読み出した行の改行文字を残します。 size 引数を負でない値にすると、最大
何バイトまで読み出すかを指定します (その結果、一行を完全に読み出さずに
データを返すことがあります)。EOFに到達すると空文字列を返します。"""),

],


'file.readlines': [
('18093981824bed6dfd6c137be5987035', True, 
"""readlines([size]) はファイル中の各行からなる文字列のリストを返します。

readine() を何度も呼び出して、読み出した行データからなるリストを返しま
す。
オプションの size 引数は、読み出すデータの合計のバイト長のだいたいの上
限を指定します。"""),
...
"""seek(offset[, whence]) はファイルの読み書き位置を移動して、None を
返します。
 
引数 offset はバイト単位で指定します。オプションの引数 whence は、デフォ
ルトでは 0 (ファイルの先頭からのオフセット。 offset >= 0 にせねばなり
ません) です。その他には、 1 (ファイルの現在の場所からの相対です。正負
どちらの値も取れます) および 2 (ファイルの末尾からの相対です、通常は負
の値をとりますが、プラットフォームによっては、ファイルの末尾を超えてファ
イルをシークできます) があります。ファイルをテキストモードで開いている
場合には、 offset に指定する値として信用できるのは tell() の示す値だけ
です。それ以外の値を指定した場合の動作は未定義です。
...
"""tell() は現在のファイル位置を整数 (長整数の場合もあります) で返しま
す。"""),
...
"""truncate([size]) は、ファイルを指定 size バイトまでに切り詰めて
None を返します。

size のデフォルト値は現在のファイル位置です。ファイル位置には tell()
の返す値を使います。"""),
...
バッファリングが行われている場合、書き込んだデータをディスク上のファイ
ルに反映させるために flush() や close() が必要な場合があるので注意して
下さい。"""),
...
"""writelines(sequence_of_strings) は複数の文字列をファイルに書きこん
で None を返します。

改行は付加されないので注意してください。 sequence_of_strings には文字
列を逐次生成するイテレーション可能オブジェクトなら何でも指定できます。
各文字列に対して繰り返し write()  を呼び出すのと同じです。"""),
...
以前のバージョンとの互換性のために存在しています。現在のファイルオブジェ
クトには、 xreadlines モジュールが実装してたのと同じパフォーマンス最適
化の仕組みが入っています。"""),
...
sequence の要素の中から、 function(item) が真であるものだけを返します。
function を None にすると、item の値が真であるもののを返します。
sequence がタプルか文字列の場合には同じ型で、それ以外の場合はリスト型
の値を返します。"""),
...
通常、この関数が必要になることはありません。主に、 Python のテストスイー
ト用に存在している関数です。

typestr は \'double\' または \'float\' です。この関数は、 typestr の示
す型として C で定義されている浮動小数点数のフォーマットが、
\'unknown\', \'IEEE, big-endian\', \'IEEE, little-endian\' のどれに一
番近いかを調べて返します。"""),
...
typestr は \'double\' または \'float\' です。 fmt には \'unknown\',
\'IEEE, big-endian\', \'IEEE, little-endian\' のいずれかを指定します。
さらに、後者の二つは、実際に C で使われているフォーマットに一致してい
る方しか使えません。

この関数は、 C レベルの浮動小数点型の自動判別の設定をオーバライドしま
す。この関数は、 float 型の値とバイナリエンコード文字列との間の変換方
法を制御します。"""),
...
もとの float の値と等しい分数を表す整数のペアを、分母が常に正の値をと
るようにして返します。値が無限大 (infinity) の場合には OverflowError
を、 NaN の場合には ValueError を送出します。
...
'frozenset': [
('599b8604dc26a205cc6823eb41629254', True, 
"""frozenset(iterable) --> frozenset オブジェクト

変更不能で、一意な要素からなる順序づけのないコレクションを生成して返し
ます。"""),
...
'frozenset.__and__': [
('34f22603582785806906d3a619fbff20', True, 
"""x.__and__(y) <==> x&y"""),
...
('ffd9e4fc21822f7b05e9a9147e7ab5cb', True, 
"""x.__contains__(y) <==> y in x."""),
...
'frozenset.__or__': [
('5aaa9cc7414a761ca43f7b6098059d20', True, 
"""x.__or__(y) <==> x|y"""),
...
'frozenset.__rand__': [
('4e31b1b862f6ec29e32800270e841ea4', True, 
"""x.__rand__(y) <==> y&x"""),
...
('588c3883072aa7e007926eb32098cdfc', True, 
"""pickle 化に必要な状態情報を返します。"""),

],


'frozenset.__ror__': [
('da4d0348ca64c8c3d0aced68631472e6', True, 
"""x.__ror__(y) <==> y|x"""),
...
'frozenset.__rxor__': [
('70015a9fed00e657f349025f83450c30', True, 
"""x.__rxor__(y) <==> y^x"""),
...
('e2b18bc0b4abb612dc21e3efe830695b', True, 
"""S.__sizeof__() は S のメモリ上のサイズをバイト単位で返します。"""),
...
'frozenset.__xor__': [
('6c6511aa3cefcc142940d3d29d9fc4dd', True, 
"""x.__xor__(y) <==> x^y"""),
...
'frozenset.copy': [
('42a24c7d25e5a9cfeeb1414761b60e42', True, 
"""浅いコピーを生成して返します。"""),
...
('2fb7b3bddc0bd6ec5ecba2d509250429', True, 
"""二つの set の交差 (intersection) からなる新たな set を生成して返し
ます。

(交差は、二つの set の両方に含まれている要素の集合です。)"""),

],


'frozenset.isdisjoint': [
('a4982a4d169a3b3f6a46cd0f44c79bc7', True, 
"""二つの set の交差が空集合のときに True を返します。"""),
...
何らかの数値を表記した文字列 S に対して、文字列の長さが width に
なるまで左側にゼロを付加します。 S を切り詰めることはありません。"""),
...
引数なしで呼び出した場合は locals() と同じです。引数を指定した
場合は、 object.__dict__ と同じです。"""),
...
"""xrange([start,] stop[, step]) -> xrange オブジェクト

range() に似ていますが、リストではなく、必要に応じて値を生成する
オブジェクトを返します。ループ中で使うと、 range() より少し高速で、
メモリの使用を抑えられます。"""),
...
タプルからなるリストを返します。各タプルには、引数に指定した各
シーケンス中のi 番目の要素が入ります。リストは、引数のシーケンス
のうち、もっとも短いもの長さに切り詰められます。"""),
