...
注意: None は `nil\' オブジェクトを表現します。また、 Ellipsis はスライスの `...\' を表します。"""),
...
モジュールを import します。引数 globals は import 時のコンテキストを決定するためだけに使われ、変更されることはありません。 locals は今のところ使われていません。 fromlist は ``from name import ...\'\' のエミュレーションをするときの import する名前のリストです。 ``import name\'\' をエミュレーションするときには、空のリストにします。モジュールをパッケージから import する場合、 fromlist を空にしておくと、 __import__(\'A.B\', ...) はパッケージ A を返します。 fromlist が空でなければ、サブモジュール B を返します。 level は import の形式が絶対import か、相対 import かを決めます。 -1 にすると、元々の import 戦略、すなわち絶対と相対の両方の import を試みます。 0 にすると絶対 import のみ、正の値にすると、現在のモジュールから指定した数だけ上のディレクトリからの相対でモジュールを探します。"""),
...
iterable のすべての要素 x について、 bool(x) が True である場合に True を返します。"""),
...
iterable のいずれかの要素 x について bool(x) が True のとき True を返します。"""),
...
呼び出し可能 (callable) なオブジェクト object を呼び出します。このとき、args にタプルを指定すると固定引数として、 kwargs に辞書を指定するとオプションのキーワード引数として使います。クラスや、 __call__() メソッドを備えたインスタンスも callable です。

リリース 2.3 で撤廃されました。 apply の代わりに、 function(*args, **keywords) を使ってください。"""),
...
"""basestring は直接インスタンス化できません。このクラスは str や unicode のベースクラスです。"""),
...
引数 x の値が真値のときに True を、それ以外のときには False を返します。 bool 型のインスタンスは、組み込み定数 True か False しかありません。 bool 型は int 型のサブクラスで、これ以上サブクラス化できません。"""),
...
引数に指定したオブジェクトを参照する、新たなバッファ (buffer) オブジェクトを生成します。生成されたバッファは、対象オブジェクトの先頭から(または offset から) のスライスを参照します。このスライスの範囲は、対象オブジェクトの末尾まで (または size に指定した長さまで) です。"""),
...
"""T.__new__(S, ...) は、 T のサブタイプ S の新たなオブジェクトを生成して返します。"""),
...
それぞれ、以下の形式のデータから、変更可能なバイト列 (bytearray) を生成します。:
...
B の両側に fillchar に指定した文字を追加して、幅が width で B が中央に位置するような文字列を生成して返します。 fillchar を省略した場合、スペースを代わりに使います。"""),
...
B[start:end] の中で、 sub 互いに重ならないで現れる数を計算して返します。 start と end はスライス表記と同じ方法で指定でき、省略するとそれぞれ B の先頭と末尾を表します。"""),
...
encoding の名前で登録されている codec を使って、B をデコードして返します。 encoding を省略すると、デフォルトエンコーディングを使います。エラーを指定すると、デコード中に発生したエラーの処理スキームを変更できます。
デフォルトの設定は \'strict\' で、すべてのエンコーディングエラーに対して UnicodeDecodeError を送出します。他には \'ignore\' と \'replace\' を指定できます。また、 UnicodeDecodeError を処理できるエラーハンドラを定義して codecs.register_error である名前で登録しておけば、その名前も指定できます。"""),
...
B が suffix で終わる場合には True を、それ以外の場合には False を返します。start を指定すると検索の開始位置を、 end を指定すると検索の終了位置を指定できます。 suffix を文字列のタプルにすると、複数の文字列を調べられます。"""),
...
S のコピーを生成し、S の中に含まれるすべてのタブ文字を複数のスペースに変換して返します。 tabsize を指定しない場合、タブの幅を 8 スペースとみなします。"""),
...
B の中の範囲 s[start, end] で部分列 sub を探し、最初に見つかった場所のインデクスを返します。 start と end は省略可能で、スライス表記と同じ指定方法を使えます。見つからなければ -1 を返します。"""),
...
16 表記で数を表現している文字列からbytearray オブジェクトを生成して返します。数の間にスペースが入っていてもかまいません。
...
B.find() と似ていますが、 sub が見つからないときに ValueError を送出します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が英数字である場合に True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字がアルファベットである場合に True を返します。それ以外の場合には False を返します。
"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が数字である場合に True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が小文字である場合に True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が空白文字である場合に True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、タイトルケースである場合、すなわち先頭の文字が大文字で、それ以降の文字が小文字である場合に True を返します。それ以外の場合には False を返します。"""),
...
B が少なくとも 1 文字からなり、そのすべての文字が大文字である場合に True を返します。それ以外の場合には False を返します。"""),
...
iterable_of_bytes の任意個数の bytearray オブジェクトを B で結合したバイト列を生成して返します。"""),
...
B を幅 width の文字列中に左寄せに配置した文字列を生成して返します。文字列は fillchar に指定した文字でパディングされます。 fillchar を省略すると、スペースを使います。"""),
...
B のコピーを生成し、先頭から、引数 bytes に指定した文字を除去して返します。引数を省略すると、先頭の ASCII 空白文字を除去します。"""),
...
S の中でセパレータ sep を探し、見つかった場合には、セパレータ以前前の文字列、セパレータ本体、セパレータ以降の文字列からなるタプルを返します。セパレータが見つからなかった場合には、 S と二つの空の bytearray からなるタプルを返します。"""),
...
B の index の位置にある要素を一つ除去し、除去した要素を返します。index を省略すると、末尾の値を pop します。"""),
...
B のコピーを生成し、 old を new に置き換えて返します。オプションの引数 count を指定した場合、先頭から count 回までで置き換えを止めます。"""),
...
B の中で sub を探し、最後に見つかった位置のインデクスを返します。sub の検索範囲は s[start,end] です。 start および end は省略可能で、スライス表記と同じ方法で指定できます。
検索に失敗すると -1 を返します。"""),
...
B.rfind() に似ていますが、見つからなかった場合に ValueError を送出します。"""),
...
B を幅 width の文字列中に右寄せに配置した文字列を生成して返します。文字列は fillchar に指定した文字でパディングされます。 fillchar を省略すると、スペースを使います。"""),
...
S の中で末尾からセパレータ sep を探し、見つかった場合には、セパレータ以前前の文字列、セパレータ本体、セパレータ以降の文字列からなるタプルを返します。セパレータが見つからなかった場合には、 S と二つの空の bytearray からなるタプルを返します。"""),
...
引数 sep に含まれる文字を区切り文字として、 B を分割したリストを生成して返します。分割は B の末尾から先頭に向けて実行されます。 sep を指定しない場合、 ASCII 空白文字 (スペース、タブ、復帰、改行、行送り、水平タブ) を使って B を分割します。 maxsplit を指定すると、maxsplit 個まで分割した時点で分割を止めます。"""),
...
B の末尾から、 bytes に指定した文字を除去して返します。bytes を省略すると、 ASCII 空白文字を除去します。"""),
...
引数 sep に含まれる文字を区切り文字として、 B を 分割したリストを生成して返します。 sep を指定しない場合、 ASCII 空白文字 (スペース、タブ、復帰、改行、行送り、水平タブ) を使って B を分割します。maxsplit を指定すると、 maxsplit 個まで分割した時点で分割を止めます。"""),
...
B を改行で区切ったリストを返します。 keepends に True を指定しない限り、リストには改行文字を含めません。"""),
...
B の部分文字列が prefix に指定した文字列から開始する場合には True、それ以外の場合には False を返します。引数 start と end には、検索の開始位置と終了位置を指定します。省略すると、それぞれ B の先頭から末尾までを調べます。 prefix には、調べたい複数の文字列をタプルで指定できます。"""),
...
文字列の先頭と末尾から、 bytes に含まれる文字を除去します。bytes 引数を省略した場合、 ASCII の空白文字を除去します。"""),
...
B のコピーを生成し、文字列中の ASCII 文字について、大文字は小文字に、小文字は大文字にして返します。"""),
...
B をタイトルケースに変換して返します。タイトルケースとは、先頭の文字が大文字で、それ以降の文字がすべて小文字であるような文字列です。"""),
...
B のコピーを生成し、deletechars が指定されている場合には deletechars 内に含まれる文字をすべて除去し、残りの文字について、変換テーブル table に従って文字を変換して返します。 table は長さ 256 のバイト列でなければなりません。"""),
...
B の左側にゼロを追加して、全体で幅 width になるようにします。B の幅が width 以下の場合でも、 B を切り詰めることはありません。"""),
...
呼出し可能オブジェクト (関数など) かどうかを判定して返します。クラスや、 __call__()  メソッドを持つインスタンスも呼出し可能です。"""),
...
インスタンスメソッドの第一引数にインスタンスを渡すように、クラスメソッドの第一引数には、クラスを渡すことになっています。クラスメソッドを宣言するには、以下のようなイディオムを使いましょう:

  class C:
      def f(cls, arg1, arg2, ...): ...
      f = classmethod(f)

クラスメソッドは、 (C.f() のような) クラスに対して呼び出す他に、 (C().f() のような) インスタンスに対しても呼び出せます。後者の場合、クラスメソッドにはインスタンスのクラス以外の情報は渡りません。クラスメソッドをサブクラスに対して呼び出すと、第一引数にはサブクラスのクラスオブジェクトが渡されます。

クラスメソッドは C++ や Java の静的メソッドとは違います。静的メソッドが必要なら、組込み関数 staticmethod を使ってください。"""),
...
二つの数値型を引数にとり、それぞれの引数を共通の型に変換して、タプルにして返します。変換には数値演算の際に使われるのと同じ規則が適用されます。型強制が不可能なときには TypeError を送出します。"""),
...
ソースコード文字列 (Python モジュールや実行文、式) をコンパイルして、exec 文や eval() で実行可能なコードオブジェクトに変換します。filename 引数は、実行時のエラーメッセージに使われます。 mode 引数は、モジュールをコンパイルする場合は \'exec\' に、単一の (対話的に実行する) 式の場合は \'single\' に、式の場合には \'eval\' にします。flags 引数を指定すると、コードのコンパイルにどの future 文を影響させるかを制御できます。
dont_inherit 引数をゼロでない値にすると、 compile を呼び出す側のコードで有効になっている future 文の効果をコンパイル処理に影響させません。dont_inherit を省略したり、ゼロを指定すると、呼び出し側の future と、明示的に指定した future の両方を考慮したコンパイルが実行されます。"""),
...
指定のアトリビュートをオブジェクトから除去します。 delattr(x, \'y\') は ``del x.y\'\' と同じです。"""),
...
"""dict() は新しい空の辞書を返します。
dict(mapping) は新しい辞書を生成し、 mapping オブジェクトのキー/値ペアを使って初期化します。
dict(seq) は新しい辞書を生成し、以下のような初期化を行います:
    d = {}
    for k, v in seq:
        d[k] = v
dict(**kwargs) は新しい辞書を生成し、キーワード引数リストの name=value ペアで初期化します。例えば、 dict(one=1, two=2) のように使います。"""),
...
"""D.__contains__(k) は、 D にキー k があれば True を、そうでなければ False を返します。"""),
...
"""dict.fromkeys(S[,v]) は 新たな辞書を返します。この辞書は S からキーを引き継ぎます。値はすべて v に設定されます。 v のデフォルト値は None です。"""),
...
"""D.get(k[,d]) は D 上にキー k があれば D[k], そうでなければ d を返します。d のデフォルト値は None です。"""),
...
辞書からキー k を除去して、キーに対応する値を返します。キーが辞書中にない場合、 d を指定していれば返し、指定していなければ KeyError を送出します。"""),
